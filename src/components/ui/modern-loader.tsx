...
const ModernLoader: React.FC<ModernLoaderProps> = ({
  ...
  return (
    <div className={cn('w-full max-w-md mx-auto p-8', className)}>
      <motion.div
        initial={{ opacity: 0, scale: 0.95 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 0.3 }}
        className={cn(
          'relative bg-background h-75 rounded-2xl shadow-2xl overflow-hidden border border-border',
          panelClassName,
        )}
      >
        <div className='px-4 py-3 flex items-center z-10 relative'>
          ...
          >
            <TypeAnimation
              ...
              deletingSpeed='slow'
              pauseDuration={2000}
              className='text-xs font-mono text-zinc-700 dark:text-zinc-400'
            />
          </motion.div>
        </div>
        ...
        {children ? (
          <div
            className={cn(
              'relative h-[calc(100%-48px)] overflow-y-auto px-5 py-4',
              contentClassName,
            )}
          >
            {children}
